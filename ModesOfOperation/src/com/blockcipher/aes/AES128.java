package com.blockcipher.aes;

import java.io.UnsupportedEncodingException;
import java.math.BigInteger;
import java.nio.charset.StandardCharsets;

/**
 * reference : http://snipplr.com/view/67929/
 * the plain text must be exactly 16-byte long
 * @author ZhengYang
 *
 */
public class AES128 {
	
	//static byte[][] state = new byte[4][4];
	static byte[] originalKey = new byte[16];
	static byte[] expandedKey = new byte[176];
	
	
	/**
	 *  Rijndael S-BOX
	 */
	private static final char SBOX[] = {
		 
	};
	 
	/**
	 *   inverted Rijndael S-BOX
	 */
	private static final char RSBOX[] = { 
		 
	};
	

	// The round constant word array, Rcon[i], contains the values given by 
	// x to th e power (i-1) being powers of x (x is denoted as {02}) in the field GF(28)
	// Note that i starts at 1, not 0).
	private static final int[] RC = {
		  
		};
	
	/*--------------------------------------------------*/
	

	/**
	 * 
	 * @param word
	 * @return
	 */
	private static byte[] rotWord(byte[] word){
		byte tmp = word[0];
		for(int i = 0; i < word.length; i++){
			word[i] = word[(i + 1 )% 4];
		}
		word[word.length - 1] = tmp;
		return word;
	}
	

	private static byte[] subWord(byte[] word){
		for (int i = 0; i < word.length; i++) {
			word[i] = (byte) SBOX[word[i] & 0xFF];
		}
		return word;
	}
	/**
	 * Expand the original 16-byte key to 176-byte key.
	 * @param originalKey
	 * @return expandedKey
	 */
	private static byte[] keyExpansion(byte[] originalKey){
		//byte[] expandedKey = new byte[176];
		byte[] tmp = new byte[4];
		for(int i = 0; i < 16; i++){
			expandedKey[i] = originalKey[i];
		}
		for(int i = 16; i < 176 - 3; i++){
			//tmp = w[i - 1]
			for(int k = 0; k < 4; k++){
				tmp[k] = expandedKey[i - 4 + k];
			}
			
			if(i % 16 == 0){
				rotWord(tmp);
				subWord(tmp);
				byte[] Rcon = {(byte) RC[i/16],0x00,0x00,0x00};
				for(int k = 0; k < 4; k++){
					tmp[k] ^= Rcon[k];
				}
			}
			if(i % 4 == 0){
				for(int k = 0; k < 4; k++){
					expandedKey[i + k] = (byte) (expandedKey[i - 16 + k] ^ tmp[k]);
				}
			}
			
			
		}
		return expandedKey;
	}
	
	/**
	 * 
	 * @param state
	 * @param invert
	 * @return state
	 */
	private static byte[][] subBytes(byte[][] state,  boolean invert){
		char[] box = invert ? RSBOX : SBOX;
		for(int i = 0; i < 4; i++){
			for(int j = 0; j < 4; j++){
				state[i][j] = (byte) box[state[i][j] & 0xFF]; //convert to integer
			}
		}
		return state;
		
	}
	 
	/**
	 * 
	 * @param state
	 * @param invert
	 * @return state
	 */
	private static byte[][] shiftRows(byte[][] state, boolean invert){
		byte[] tmp = new byte[4];

		for(int i = 0; i < 4; i++){
			for(int j = 0; j < 4; j++){
				tmp[j] = state[i][invert ? (j - i + 4) % 4 : (j + i) % 4];
			}
			for(int j = 0; j < 4; j++){
				state[i][j] = tmp[j];
			}	
		}
		
		return state;
	}
	
	
	
	// Multiplication function over GF(2^8) used in the MixColumns pass
	public static byte GFMult(byte a, byte b) {
		byte r = 0, t;
		while (a != 0) {
			if ((a & 1) != 0)
				r = (byte) (r ^ b); 
			t = (byte) (b & 0x80); // t (b7)
			b = (byte) (b << 1); // b left shift
			if (t != 0)
				b = (byte) (b ^ 0x1b); //if b7==1, b XOR 0001 1011 
			a = (byte) ((a & 0xff) >> 1);
		}
		return r;
	}
	
	/**
	 *  AES MixColums and InvMixColumns passes
	 * @param state
	 * @param inverted
	 * @return 
	 */
	private static byte[][] mixColumns(byte[][] state, boolean inverted) {
		int[] tmp = new int[4];

		byte a = (byte) (inverted ? 0x0b : 0x03);
		byte b = (byte) (inverted ? 0x0d : 0x01);
		byte c = (byte) (inverted ? 0x09 : 0x01);
		byte d = (byte) (inverted ? 0x0e : 0x02);
 
		for (int i = 0; i < 4; i++) {
			tmp[0] = GFMult(d, state[0][i]) ^ GFMult(a, state[1][i]) ^ GFMult(b, state[2][i]) ^ GFMult(c, state[3][i]);
			tmp[1] = GFMult(c, state[0][i]) ^ GFMult(d, state[1][i]) ^ GFMult(a, state[2][i]) ^ GFMult(b, state[3][i]);
			tmp[2] = GFMult(b, state[0][i]) ^ GFMult(c, state[1][i]) ^ GFMult(d, state[2][i]) ^ GFMult(a, state[3][i]);
			tmp[3] = GFMult(a, state[0][i]) ^ GFMult(b, state[1][i]) ^ GFMult(c, state[2][i]) ^ GFMult(d, state[3][i]);
			for (int j = 0; j < 4; j++)
				state[j][i] = (byte) (tmp[j]);
		}
 
		return state;
	}
	
	
	/**
	 * addRoundKey
	 * @param args
	 */
	private static byte[][] addRoundKeys(byte[][] state, int round){
		byte[][] tmp = new byte[4][4];
		for(int i = 0; i < 4; i++){
			for(int j = 0; j < 4; j++){
				tmp[j][i] = expandedKey[i * 4 + j + round * 16];
			}
		}
		for(int i = 0; i < 4; i++){
			for(int j = 0; j < 4; j++){
				state[i][j] ^= tmp[i][j];
			}
		}
		return state;
	}

	
	/**
	 * encrypt the plaintext
	 * @param state
	 * @return
	 */
	public static byte[][] aesEncrypt(byte[][] state){	
		keyExpansion(originalKey);
		addRoundKeys(state, 0);
		for(int k = 1; k < 10 ; k++){
			subBytes(state, false);
			shiftRows(state, false);
			mixColumns(state, false);
			addRoundKeys(state, k);
		}
		subBytes(state, false);
		shiftRows(state, false);
		addRoundKeys(state, 10);
		
		return state;
	}

	/**
	 * decrypt the ciphertext
	 * @param state
	 * @return
	 */
	public static byte[][] aesDecrpyt(byte[][] state){
		keyExpansion(originalKey);
		addRoundKeys(state, 10);
		for(int k = 9; k > 0; k--){
			shiftRows(state, true);
			subBytes(state, true);
			addRoundKeys(state, k);
			mixColumns(state, true);
		}
		shiftRows(state, true);
		subBytes(state, true);
		addRoundKeys(state, 0);
		
		return state;
	}
	
	
	public static void show(byte[][] state){
		for (byte[] bs : state) {
			for (int i = 0; i < bs.length; i++) {
				System.out.print(String.format("%02X",bs[i]) + " ");
			}
			System.out.println();
		}
		System.out.println();
	}
	

	public static byte[] strToByte(String pt){
		String hexString = String.format("%x", new BigInteger(1, pt.getBytes()));
		int len = hexString.length();
		byte[] data = new byte[len / 2];
		for (int i = 0; i < len; i += 2) {
			data[i / 2] = (byte) ((Character.digit(hexString.charAt(i), 16) << 4) + Character.digit(
					hexString.charAt(i + 1), 16));
		}
		return data;
	}
	
	public static byte[][] byteToState(byte[] data){
		byte[][] matrix = new byte[4][4];
		for (int i = 0; i < 4; i++)
			for (int j = 0; j < 4; j++)
				matrix[i][j] = data[i * 4 + j];
		return matrix;
	}
	
	public static byte[][] strToState(String pt){
		return byteToState(strToByte(pt));
	}
	
	public static String stateToHexstr(byte[][] state){
		byte[] array = new byte[16];
		for (int i = 0; i < 4; i++)
			for (int j = 0; j < 4; j++)
				array[i * 4 + j] = state[i][j];
		
		String hexString = new String();
		for (byte hex : array)
			hexString += Integer.toString(hex & 0xFF, 16);
 
		return hexString;
	}
	
	public static String stringToHexString(String string) {
		return String.format("%x", new BigInteger(1, string.getBytes()));
	}
	
	
	public static String hexToString(String hexstring) {
		String str = "";
		for(int i = 0; i < hexstring.length();i += 2)
		{
	        String s = hexstring.substring(i, (i + 2));
	        int decimal = Integer.parseInt(s, 16);
	        str = str + (char) decimal;
		}       
		return str;
	}
	

	public static byte[] matrixToArray(byte[][] matrix) {
		byte[] array = new byte[16];
		for (int i = 0; i < 4; i++)
			for (int j = 0; j < 4; j++)
				array[i * 4 + j] = matrix[i][j];
		return array;
	}
	/*--------------------------------------------------*/
	public static byte[] getOriginalKey() {
		return originalKey;
	}

	public static void setOriginalKey(byte[] originalKey) {
		AES128.originalKey = originalKey;
	}
	
	/*--------------------------------------------------*/
	

}
